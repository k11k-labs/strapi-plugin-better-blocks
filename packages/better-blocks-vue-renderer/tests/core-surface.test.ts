 ...
 * re-exports from `@qkix/better-blocks-core`. The React and Astro renderers run
 * the same suite: the core is shared, so the three renderers have to agree on
 * what it does, and a change to it fails here rather than in one renderer's
 * output.
 */
