...
import CustomAudio from './fixtures/CustomAudio.vue';
import CustomBg from './fixtures/CustomBg.vue';
import CustomBold from './fixtures/CustomBold.vue';
import CustomButton from './fixtures/CustomButton.vue';
import CustomCallout from './fixtures/CustomCallout.vue';
import CustomCode from './fixtures/CustomCode.vue';
import CustomColor from './fixtures/CustomColor.vue';
import CustomDetails from './fixtures/CustomDetails.vue';
import CustomDiagram from './fixtures/CustomDiagram.vue';
import CustomEmbed from './fixtures/CustomEmbed.vue';
import CustomEmbedBlock from './fixtures/CustomEmbedBlock.vue';
import CustomHeading from './fixtures/CustomHeading.vue';
import CustomHr from './fixtures/CustomHr.vue';
import CustomImage from './fixtures/CustomImage.vue';
import CustomLink from './fixtures/CustomLink.vue';
import CustomMath from './fixtures/CustomMath.vue';
import CustomParagraph from './fixtures/CustomParagraph.vue';
import CustomQuote from './fixtures/CustomQuote.vue';
import CustomRow from './fixtures/CustomRow.vue';
import CustomSocialEmbed from './fixtures/CustomSocialEmbed.vue';
import CustomTable from './fixtures/CustomTable.vue';
import CustomTd from './fixtures/CustomTd.vue';
import CustomTh from './fixtures/CustomTh.vue';
import CustomVideo from './fixtures/CustomVideo.vue';
...
/**
 * Renders through Vue's server renderer - the markup a Nuxt page actually ships
 * - and parses it into a DOM to assert against. Anything the components do on
 * the client (code highlighting, social widget scripts) is tested by mounting
 * them instead, further down.
 */
async function render(content: BlocksContent | null, props: RenderProps = {}) {
  const app = createSSRApp({ render: () => h(BlocksRenderer, { content, ...props }) });
  const raw = await renderToString(app);
  // Vue marks fragments and unrendered branches with comment anchors. They
  // carry no markup and would only get in the way of the assertions.
  const html = raw.replace(/<!--(?:\[|\])?-->/g, '');
  document.body.innerHTML = html;
  return { html, document, container: document.body };
}
...
describe('BlocksRenderer', () => {
  ...
  // ── Code Block ───────────────────────────────────────────────────
  //
  // Shiki cannot highlight synchronously, so the server (and the first client
  // render) emit the raw source in a plain <pre>. Highlighting is a client
  // concern and is tested by mounting, in the "on the client" block below.
  ...
  it('wraps code blocks in a bb-code container', async () => {
    ...
    expect(container.querySelector('.bb-code pre.bb-code-pre')).not.toBeNull();
  });

  it('server-renders the source verbatim, so hydration has something to match', async () => {
    const { container } = await render([
      { type: 'code', language: 'typescript', children: [{ type: 'text', text: 'const x = 1;' }] },
    ]);
    expect(container.querySelector('.bb-code-pre code')?.textContent).toBe('const x = 1;');
  });
  ...
  it('omits the copy button by default', async () => {
    ...
    expect(container.querySelector('.bb-code-copy')).toBeNull();
  });
  ...
  it('renders an opt-in copy button when codeCopyButton is set', async () => {
    ...
    expect(container.querySelector('.bb-code button.bb-code-copy')).not.toBeNull();
  });
  ...
  it('uses a custom code renderer with plainText and language', async () => {
    ...
    expect(container.querySelector('.bb-code')).toBeNull();
  });
  ...
  it('keeps the native download attribute on a download-mode file button', async () => {
    const { container } = await render([
      {
        type: 'button',
        buttonType: 'file',
    ...
    const a = container.querySelector('a.bb-button');
    // The native attribute works same-origin on its own; cross-origin is where
    // the click handler takes over (mounted test further down).
