...
import type { ButtonFile, InlineNode, TextNode } from '../src';

/**
 * The same characterization suite the two renderers run, now asserted directly
 * against the core. The expectations are unchanged from the ones that ran on
 * each renderer before the extraction, which is what makes them evidence that
